...
1818  03e6 200e          	jra	L375
1819  03e8               L175:
1820                     ; 579     flagstatus = (uint8_t)(FLASH->IAPSR & (FLASH_IAPSR_EOP | FLASH_IAPSR_WR_PG_DIS));
1822  03e8 c6505f        	ld	a,20575
1823  03eb a405          	and	a,#5
1824  03ed 6b03          	ld	(OFST+0,sp),a
1826                     ; 580     timeout--;
1828  03ef 1e01          	ldw	x,(OFST-2,sp)
1829  03f1 1d0001        	subw	x,#1
1830  03f4 1f01          	ldw	(OFST-2,sp),x
1832  03f6               L375:
1833                     ; 577   while((flagstatus == 0x00) && (timeout != 0x00))
1835  03f6 0d03          	tnz	(OFST+0,sp)
1836  03f8 2604          	jrne	L775
1838  03fa 1e01          	ldw	x,(OFST-2,sp)
1839  03fc 26ea          	jrne	L175
1840  03fe               L775:
1841                     ; 584   if(timeout == 0x00 )
1843  03fe 1e01          	ldw	x,(OFST-2,sp)
1844  0400 2604          	jrne	L106
1845                     ; 586     flagstatus = FLASH_STATUS_TIMEOUT;
1847  0402 a602          	ld	a,#2
1848  0404 6b03          	ld	(OFST+0,sp),a
1850  0406               L106:
1851                     ; 589   return((FLASH_Status_TypeDef)flagstatus);
1853  0406 7b03          	ld	a,(OFST+0,sp)
1856  0408 5b03          	addw	sp,#3
1857  040a 81            	ret
1921                     ; 599 IN_RAM(void FLASH_EraseBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType))
1921                     ; 600 {
1922                     	switch	.text
1923  040b               _FLASH_EraseBlock:
1925  040b 89            	pushw	x
1926  040c 5206          	subw	sp,#6
1927       00000006      OFST:	set	6
1930                     ; 601   uint32_t startaddress = 0;
1932                     ; 611   assert_param(IS_MEMORY_TYPE_OK(FLASH_MemType));
1934  040e 7b0b          	ld	a,(OFST+5,sp)
1935  0410 a1fd          	cp	a,#253
1936  0412 2706          	jreq	L402
1937  0414 7b0b          	ld	a,(OFST+5,sp)
1938  0416 a1f7          	cp	a,#247
1939  0418 2603          	jrne	L202
1940  041a               L402:
1941  041a 4f            	clr	a
1942  041b 2010          	jra	L602
1943  041d               L202:
1944  041d ae0263        	ldw	x,#611
1945  0420 89            	pushw	x
1946  0421 ae0000        	ldw	x,#0
1947  0424 89            	pushw	x
1948  0425 ae0010        	ldw	x,#L73
1949  0428 cd0000        	call	_assert_failed
1951  042b 5b04          	addw	sp,#4
1952  042d               L602:
1953                     ; 612   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
1955  042d 7b0b          	ld	a,(OFST+5,sp)
1956  042f a1fd          	cp	a,#253
1957  0431 2626          	jrne	L536
1958                     ; 614     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
1960  0433 1e07          	ldw	x,(OFST+1,sp)
1961  0435 a30080        	cpw	x,#128
1962  0438 2403          	jruge	L012
1963  043a 4f            	clr	a
1964  043b 2010          	jra	L212
1965  043d               L012:
1966  043d ae0266        	ldw	x,#614
1967  0440 89            	pushw	x
1968  0441 ae0000        	ldw	x,#0
1969  0444 89            	pushw	x
1970  0445 ae0010        	ldw	x,#L73
1971  0448 cd0000        	call	_assert_failed
1973  044b 5b04          	addw	sp,#4
1974  044d               L212:
1975                     ; 615     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
1977  044d ae8000        	ldw	x,#32768
1978  0450 1f05          	ldw	(OFST-1,sp),x
1979  0452 ae0000        	ldw	x,#0
1980  0455 1f03          	ldw	(OFST-3,sp),x
1983  0457 2024          	jra	L736
1984  0459               L536:
1985                     ; 619     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
1987  0459 1e07          	ldw	x,(OFST+1,sp)
1988  045b a3000a        	cpw	x,#10
1989  045e 2403          	jruge	L412
1990  0460 4f            	clr	a
1991  0461 2010          	jra	L612
1992  0463               L412:
1993  0463 ae026b        	ldw	x,#619
1994  0466 89            	pushw	x
1995  0467 ae0000        	ldw	x,#0
1996  046a 89            	pushw	x
1997  046b ae0010        	ldw	x,#L73
1998  046e cd0000        	call	_assert_failed
2000  0471 5b04          	addw	sp,#4
2001  0473               L612:
2002                     ; 620     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
2004  0473 ae4000        	ldw	x,#16384
2005  0476 1f05          	ldw	(OFST-1,sp),x
2006  0478 ae0000        	ldw	x,#0
2007  047b 1f03          	ldw	(OFST-3,sp),x
2009  047d               L736:
2010                     ; 628     pwFlash = (PointerAttr uint32_t *)(MemoryAddressCast)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
2012  047d 1e07          	ldw	x,(OFST+1,sp)
2013  047f a640          	ld	a,#64
2014  0481 cd0000        	call	c_cmulx
2016  0484 96            	ldw	x,sp
2017  0485 1c0003        	addw	x,#OFST-3
2018  0488 cd0000        	call	c_ladd
2020  048b be02          	ldw	x,c_lreg+2
2021  048d 1f01          	ldw	(OFST-5,sp),x
2023                     ; 632   FLASH->CR2 |= FLASH_CR2_ERASE;
2025  048f 721a505b      	bset	20571,#5
2026                     ; 633   FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NERASE);
2028  0493 721b505c      	bres	20572,#5
2029                     ; 637     *pwFlash = (uint32_t)0;
2031  0497 1e01          	ldw	x,(OFST-5,sp)
2032  0499 a600          	ld	a,#0
2033  049b e703          	ld	(3,x),a
2034  049d a600          	ld	a,#0
2035  049f e702          	ld	(2,x),a
2036  04a1 a600          	ld	a,#0
2037  04a3 e701          	ld	(1,x),a
2038  04a5 a600          	ld	a,#0
2039  04a7 f7            	ld	(x),a
2040                     ; 645 }
2043  04a8 5b08          	addw	sp,#8
2044  04aa 81            	ret
...
2149                     ; 658 {
2150                     	switch	.text
2151  04ab               _FLASH_ProgramBlock:
2153  04ab 89            	pushw	x
2154  04ac 5206          	subw	sp,#6
...
2164  04ae 7b0b          	ld	a,(OFST+5,sp)
2165  04b0 a1fd          	cp	a,#253
2166  04b2 2706          	jreq	L422
2167  04b4 7b0b          	ld	a,(OFST+5,sp)
2168  04b6 a1f7          	cp	a,#247
2169  04b8 2603          	jrne	L222
2170  04ba               L422:
2171  04ba 4f            	clr	a
2172  04bb 2010          	jra	L622
2173  04bd               L222:
2174  04bd ae0297        	ldw	x,#663
2175  04c0 89            	pushw	x
2176  04c1 ae0000        	ldw	x,#0
2177  04c4 89            	pushw	x
2178  04c5 ae0010        	ldw	x,#L73
2179  04c8 cd0000        	call	_assert_failed
2181  04cb 5b04          	addw	sp,#4
2182  04cd               L622:
2183                     ; 664   assert_param(IS_FLASH_PROGRAM_MODE_OK(FLASH_ProgMode));
2185  04cd 0d0c          	tnz	(OFST+6,sp)
2186  04cf 2706          	jreq	L232
2187  04d1 7b0c          	ld	a,(OFST+6,sp)
2188  04d3 a110          	cp	a,#16
2189  04d5 2603          	jrne	L032
2190  04d7               L232:
2191  04d7 4f            	clr	a
2192  04d8 2010          	jra	L432
2193  04da               L032:
2194  04da ae0298        	ldw	x,#664
2195  04dd 89            	pushw	x
2196  04de ae0000        	ldw	x,#0
2197  04e1 89            	pushw	x
2198  04e2 ae0010        	ldw	x,#L73
2199  04e5 cd0000        	call	_assert_failed
2201  04e8 5b04          	addw	sp,#4
2202  04ea               L432:
2203                     ; 665   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
2205  04ea 7b0b          	ld	a,(OFST+5,sp)
2206  04ec a1fd          	cp	a,#253
2207  04ee 2626          	jrne	L317
2208                     ; 667     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
2210  04f0 1e07          	ldw	x,(OFST+1,sp)
2211  04f2 a30080        	cpw	x,#128
2212  04f5 2403          	jruge	L632
2213  04f7 4f            	clr	a
2214  04f8 2010          	jra	L042
2215  04fa               L632:
2216  04fa ae029b        	ldw	x,#667
2217  04fd 89            	pushw	x
2218  04fe ae0000        	ldw	x,#0
2219  0501 89            	pushw	x
2220  0502 ae0010        	ldw	x,#L73
2221  0505 cd0000        	call	_assert_failed
2223  0508 5b04          	addw	sp,#4
2224  050a               L042:
2225                     ; 668     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
2227  050a ae8000        	ldw	x,#32768
2228  050d 1f03          	ldw	(OFST-3,sp),x
2229  050f ae0000        	ldw	x,#0
2230  0512 1f01          	ldw	(OFST-5,sp),x
2233  0514 2024          	jra	L517
2234  0516               L317:
2235                     ; 672     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
2237  0516 1e07          	ldw	x,(OFST+1,sp)
2238  0518 a3000a        	cpw	x,#10
2239  051b 2403          	jruge	L242
2240  051d 4f            	clr	a
2241  051e 2010          	jra	L442
2242  0520               L242:
2243  0520 ae02a0        	ldw	x,#672
2244  0523 89            	pushw	x
2245  0524 ae0000        	ldw	x,#0
2246  0527 89            	pushw	x
2247  0528 ae0010        	ldw	x,#L73
2248  052b cd0000        	call	_assert_failed
2250  052e 5b04          	addw	sp,#4
2251  0530               L442:
2252                     ; 673     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
2254  0530 ae4000        	ldw	x,#16384
2255  0533 1f03          	ldw	(OFST-3,sp),x
2256  0535 ae0000        	ldw	x,#0
2257  0538 1f01          	ldw	(OFST-5,sp),x
2259  053a               L517:
2260                     ; 677   startaddress = startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE);
2262  053a 1e07          	ldw	x,(OFST+1,sp)
2263  053c a640          	ld	a,#64
2264  053e cd0000        	call	c_cmulx
2266  0541 96            	ldw	x,sp
2267  0542 1c0001        	addw	x,#OFST-5
2268  0545 cd0000        	call	c_lgadd
2271                     ; 680   if(FLASH_ProgMode == FLASH_PROGRAMMODE_STANDARD)
2273  0548 0d0c          	tnz	(OFST+6,sp)
2274  054a 260a          	jrne	L717
2275                     ; 683     FLASH->CR2 |= FLASH_CR2_PRG;
2277  054c 7210505b      	bset	20571,#0
2278                     ; 684     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NPRG);
2280  0550 7211505c      	bres	20572,#0
2282  0554 2008          	jra	L127
2283  0556               L717:
2284                     ; 689     FLASH->CR2 |= FLASH_CR2_FPRG;
2286  0556 7218505b      	bset	20571,#4
2287                     ; 690     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NFPRG);
2289  055a 7219505c      	bres	20572,#4
2290  055e               L127:
2291                     ; 694   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
2293  055e 5f            	clrw	x
2294  055f 1f05          	ldw	(OFST-1,sp),x
2296  0561               L327:
2297                     ; 696     *((PointerAttr uint8_t*) (MemoryAddressCast)startaddress + Count) = ((uint8_t)(Buffer[Count]));
2299  0561 1e0d          	ldw	x,(OFST+7,sp)
2300  0563 72fb05        	addw	x,(OFST-1,sp)
2301  0566 f6            	ld	a,(x)
2302  0567 1e03          	ldw	x,(OFST-3,sp)
2303  0569 72fb05        	addw	x,(OFST-1,sp)
2304  056c f7            	ld	(x),a
2305                     ; 694   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
2307  056d 1e05          	ldw	x,(OFST-1,sp)
2308  056f 1c0001        	addw	x,#1
2309  0572 1f05          	ldw	(OFST-1,sp),x
2313  0574 1e05          	ldw	x,(OFST-1,sp)
2314  0576 a30040        	cpw	x,#64
2315  0579 25e6          	jrult	L327
2316                     ; 698 }
2319  057b 5b08          	addw	sp,#8
2320  057d 81            	ret
...
2354                     	switch	.const
2355  0010               L73:
2356  0010 736f75726365  	dc.b	"source files\stm8s"
2357  0022 5f666c617368  	dc.b	"_flash.c",0
2358                     	xref.b	c_lreg
2359                     	xref.b	c_x
2360                     	xref.b	c_y
2380                     	xref	c_ladd
